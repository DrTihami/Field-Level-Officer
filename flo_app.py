...
# Load the data
flo_data = pd.read_csv('FLO_File.csv')  # Make sure the file name includes '.csv'
...
if 'Latitude' in flo_data.columns and 'Longitude' in flo_data.columns:
    # Convert FLO coordinates into a numpy array for BallTree
    flo_coords = flo_data[['Latitude', 'Longitude']].values
    ...
    st.markdown(
        ...
        <div style="background-color: #E6F7FF; padding: 10px; border-radius: 5px; margin-top: 10px;">
            <h3 style="color: #004085; text-align: center; font-size: 18px;">
                Objective of Application: This application helps in locating the nearest Field Level Officer for farmers based on their latitude and longitude coordinates. JKVDA Kashmir Courtesy.
            </h3>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    if st.button("Nearest_Field Level Officer"):
        ...
        st.write(f"**FLO Name**: {nearest_flo['Field Level Officer']}")
        st.write(f"**FLO Address**: {nearest_flo['Address']}")
        st.write(f"**FLO District**: {nearest_flo['District']}")
        st.write(f"**Scrutiny Officer**: {nearest_flo['Scrutiny Officer']}")
        st.write(f"**District Implementing Officer**: {nearest_flo['Implementing Officer']}")
